...
install.packages(c("tidyverse", "r2rtf", "admiral", "pharmaversesdtm", 
                   "pharmaverseadam", "teal", "teal.data", "teal.modules.general"))
...
#--------------------------------------------------
# Custom Demographics Module with Filters
#--------------------------------------------------
    ...
    server = function(id, data) {
      moduleServer(id, function(input, output, session) {
        ...
        output$filter_ui <- renderUI({
          ...
          tagList(
            selectInput(
              ...
              "Safety Flag (SAFFL):",
              choices = c("All", unique(df$SAFFL)),
              selected = "All"
            ),
            selectInput(
              session$ns("randfl_filter"),
              "Randomized Flag (RANDFL):",
              choices = c("All", unique(df$RANDFL)),
              selected = "All"
            ),
            selectInput(
              session$ns("sex_filter"),
              "Sex:",
              choices = c("All", unique(df$SEX)),
              selected = "All"
            ),
            selectInput(
              session$ns("race_filter"),
              "Race:",
              choices = c("All", unique(df$RACE)),
              selected = "All"
            ),
            selectInput(
              session$ns("ethnic_filter"),
              "Ethnicity:",
              choices = c("All", unique(df$ETHNIC)),
              selected = "All"
            )
          )
        })
        ...
        filtered_data <- reactive({
          req(input$saffl_filter, input$randfl_filter, input$sex_filter, 
              input$race_filter, input$ethnic_filter)
          ...
          if (input$race_filter != "All") {
            df <- df %>% filter(RACE == input$race_filter)
          }
          if (input$ethnic_filter != "All") {
            df <- df %>% filter(ETHNIC == input$ethnic_filter)
          }
